...
RSpec.describe ComicBook::CBZ do
  ...
  describe '#archive' do
    ...
    it 'uses custom extension when specified' do
      output_path = adapter.archive extension: :zip
  ...
  describe '#extract' do
    ...
    it 'uses custom extension when specified' do
      extract_path = adapter.extract extension: :folder
  ...
  describe '#info' do
    context 'with ComicInfo.xml in the archive' do
      ...
      let(:test_cbz) { File.join temp_dir, 'with_comicinfo.cbz' }

      before do
        load_fixture('cbz/with_comicinfo.cbz').copy_to test_cbz
      end
