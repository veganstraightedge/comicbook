...
RSpec.describe ComicBook::CB7::Extractor do
  ...
  describe '#extract' do
    ...
    context 'when archive is empty' do
      let(:cb7_file) { load_fixture('cb7/empty.cb7') }
      let(:temp_cb7_file) { File.join temp_dir, 'empty.cb7' }
    ...
    context 'when archive contains only non-image files' do
      let(:cb7_file) { load_fixture('cb7/text_only.cb7') }
      let(:temp_cb7_file) { File.join temp_dir, 'text_only.cb7' }
